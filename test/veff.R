...
{
  C <- rep(0,Ns)
  X <- matrix(round(runif(Ns*2,min=0,max = Lx),3), ncol = 2)
  Y <- matrix(round(runif(Ns*2,min=0,max = Ly),3), ncol = 2)
  Z <- matrix(round(runif(Ns*2,min=0,max = Lz),3), ncol = 2)
  ################################################
...
for(i in seq(1,Nb,1)){
  ...
  Veff[i] <- (Lx*Ly*Lz)^2 * (sum(S) / (Ns*k))  
  FE[i] <- -log(Veff[i] /((0.5*Np)^2 * Lp^3 * Ln^3))
}
FE <- FE[is.finite(FE)]
Stats <- cbind(mean(FE),sd(FE),max(FE),min(FE))
...
#print(k)
#write.table(StatsV,file = paste0('StatsV'),row.names = F,col.names = c("mean","sd","max","min"))
#write.table(Stats,file = paste0('Stats'),row.names = F,col.names = c("mean","sd","max","min"))
